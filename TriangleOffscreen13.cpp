...
public:
	...
	// NEW: Add render target structure
	struct RenderTarget {
	...
	// NEW: Framebuffer for render target
	...
	~VulkanExample()
	{
		...
		// NEW: Clean up render target resources
		...
		vkDestroyFramebuffer(device, renderTargetFramebuffer, nullptr);
	...
	void createRenderTarget()
	{
		...
		imageInfo.extent.width = renderTarget.width;
		imageInfo.extent.height = renderTarget.height;
		...
		VK_CHECK_RESULT(vkCreateImage(device, &imageInfo, nullptr, &renderTarget.image));
		...
		VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &renderTarget.memory));
		...
		framebufferInfo.width = renderTarget.width;
		framebufferInfo.height = renderTarget.height;
		framebufferInfo.layers = 1;
		VK_CHECK_RESULT(vkCreateFramebuffer(device, &framebufferInfo, nullptr, &renderTargetFramebuffer));
	}
	...
	virtual void render()
	{
		...
		renderPassBeginInfo.framebuffer = renderTargetFramebuffer;
