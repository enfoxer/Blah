...
private:
    ...
    void createInstance() {
        ...
        // Enable the Win32 surface extension
    ...
    void createGraphicsPipeline() {
        ...
        std::vector<uint32_t> vertShaderCode = compileHLSLToSPIRV(vertexShaderSource, EShLangVertex);
        std::vector<uint32_t> fragShaderCode = compileHLSLToSPIRV(fragmentShaderSource, EShLangFragment);
    ...
    std::vector<uint32_t> compileHLSLToSPIRV(const char* source, EShLanguage stage) {
        ...
        if (!shader.parse(GetDefaultResources(), 100, false, messages)) {
            throw std::runtime_error("Failed to compile HLSL shader: " + std::string(shader.getInfoLog()));
        }
        ...
        if (!program.link(messages)) {
            throw std::runtime_error("Failed to link HLSL shader: " + std::string(program.getInfoLog()));
        }
