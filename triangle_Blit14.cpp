...
public:
	...
	// Enhanced render target structure
	struct RenderTarget {
	...
	RenderTarget secondaryRenderTarget;
    ...
    ~VulkanExample()
    {
        ...
        // Cleanup both render targets
        cleanupRenderTarget(primaryRenderTarget);
        cleanupRenderTarget(secondaryRenderTarget);
	...
	void cleanupRenderTarget(RenderTarget& rt) {
		vkDestroyFramebuffer(device, rt.framebuffer, nullptr);
		vkDestroyImageView(device, rt.view, nullptr);
		vkDestroyImage(device, rt.image, nullptr);
		vkFreeMemory(device, rt.memory, nullptr);
	}

	// Create a single render target
	void initializeRenderTargetLayout(RenderTarget& rt, VkCommandBuffer cmdBuffer) {
		...
		currentSecondaryLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
	}

	void createRenderTarget(RenderTarget& rt, VkImageUsageFlags additionalUsage = 0)

	{
		...
		// Base usage plus any additional flags
		imageInfo.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
			VK_IMAGE_USAGE_TRANSFER_DST_BIT | additionalUsage;
		...
		VK_CHECK_RESULT(vkCreateFramebuffer(device, &framebufferInfo, nullptr, &rt.framebuffer));
	}

	// Track current layout of render targets
	VkImageLayout currentPrimaryLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	VkImageLayout currentSecondaryLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		...
		VkImageAspectFlags aspectMask)
	{
		...
		// Add support for undefined to color attachment transition
	...
	void blitRenderTargets(VkCommandBuffer cmdBuffer)
	{
		// Only transition if we're not already in the correct layout
		if (currentPrimaryLayout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) {
			transitionImageLayout(
				cmdBuffer,
			...
			currentPrimaryLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
		}

		// Only transition if we're not already in the correct layout
		if (currentSecondaryLayout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
			currentPrimaryLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		}

		if (currentSecondaryLayout != VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
			transitionImageLayout(
				cmdBuffer,
				secondaryRenderTarget.image,
				VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
			...
			blit.srcOffsets[1] = { (int32_t)primaryRenderTarget.width, (int32_t)primaryRenderTarget.height, 1 };
			blit.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			blit.srcSubresource.mipLevel = 0;
			blit.srcSubresource.baseArrayLayer = 0;
			blit.srcSubresource.layerCount = 1;
			blit.dstOffsets[0] = { 0, 0, 0 };
			blit.dstOffsets[1] = { (int32_t)secondaryRenderTarget.width, (int32_t)secondaryRenderTarget.height, 1 };
			...
			vkCmdBlitImage(
				cmdBuffer,
				primaryRenderTarget.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
				secondaryRenderTarget.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
				1, &blit,
				VK_FILTER_LINEAR
			);

			currentSecondaryLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
		}

		// Transition images back to color attachment optimal
		if (currentPrimaryLayout != VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
			transitionImageLayout(
				cmdBuffer,
				primaryRenderTarget.image,
				VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
	...
	// NEW: Helper functions for command buffer management
	...
	void prepare()
	{
		...
		secondaryRenderTarget.width = 1280;
		secondaryRenderTarget.height = 800;
		createRenderTarget(secondaryRenderTarget);

		// Initialize render target layouts using the new helper functions
		VkCommandBuffer cmdBuffer = beginSingleTimeCommands();
		initializeRenderTargetLayout(primaryRenderTarget, cmdBuffer);
		initializeRenderTargetLayout(secondaryRenderTarget, cmdBuffer);
	...
	virtual void render()
	{
		...
		VkViewport viewport{};
		viewport.height = (float)primaryRenderTarget.height;
		viewport.width = (float)primaryRenderTarget.width;
		viewport.minDepth = (float)0.0f;
		viewport.maxDepth = (float)1.0f;
		vkCmdSetViewport(commandBuffer, 0, 1, &viewport);

		VkRect2D scissor{};
		scissor.extent.width = primaryRenderTarget.width;
		scissor.extent.height = primaryRenderTarget.height;
		scissor.offset.x = 0;
		scissor.offset.y = 0;
		...
		// Blit from primary to secondary render target
		blitRenderTargets(commandBuffer);
		...
		submitInfo.commandBufferCount = 1;
		submitInfo.pCommandBuffers = &commandBuffer;
		submitInfo.signalSemaphoreCount = 0;  // No need to signal since we're not presenting
		submitInfo.waitSemaphoreCount = 0;    // No need to wait since we're not presenting

		VK_CHECK_RESULT(vkQueueSubmit(queue, 1, &submitInfo, waitFences[currentFrame]));

		// Keep window system happy by acquiring and presenting a swapchain image
		uint32_t imageIndex;
